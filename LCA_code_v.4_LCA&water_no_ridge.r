...
ppm105 <- ppm(pare10, ~ pd_costpath+tot_vis, Strauss(1000), correction= 'none')
ppm106 <- ppm(pare10, ~ pd_costpath+elev, Strauss(1000), correction= 'none')
ppm107 <- ppm(pare10, ~ pd_costpath+water, Strauss(1000), correction= 'none')
ppm108 <- ppm(pare10, ~ tot_vis+elev, Strauss(1000), correction= 'none')
ppm109 <- ppm(pare10, ~ tot_vis+water, Strauss(1000), correction= 'none')
ppm110 <- ppm(pare10, ~ elev+water, Strauss(1000), correction= 'none')
ppm111 <- ppm(pare10, ~ pd_costpath+tot_vis+elev, Strauss(1000), correction= 'none')
ppm112 <- ppm(pare10, ~ pd_costpath+tot_vis+water, Strauss(1000), correction= 'none')
ppm113 <- ppm(pare10, ~ pd_costpath+elev+water, Strauss(1000), correction= 'none')
ppm114 <- ppm(pare10, ~ tot_vis+elev+water, Strauss(1000), correction= 'none')
ppm115 <- ppm(pare10, ~ pd_costpath+tot_vis+elev+water, Strauss(1000), correction= 'none')


#compare Gibbs models
ppm_AICc10 <- model.sel(ppm100, ppm101, ppm102, ppm103, ppm104, ppm105, ppm106, ppm107, ppm108, ppm109, ppm110, ppm111, ppm112, ppm113, ppm114, ppm115, rank=AICc)
ppm_AICc10
...
ppm205 <- ppm(pare20, ~ pd_costpath+tot_vis, hybrid20, correction= 'none')
ppm206 <- ppm(pare20, ~ pd_costpath+elev, hybrid20, correction= 'none')
ppm207 <- ppm(pare20, ~ pd_costpath+water, hybrid20, correction= 'none')
...
ppm211 <- ppm(pare20, ~ pd_costpath+tot_vis+elev, hybrid20, correction= 'none')
ppm212 <- ppm(pare20, ~ pd_costpath+tot_vis+water, hybrid20, correction= 'none')
ppm213 <- ppm(pare20, ~ pd_costpath+elev+water, hybrid20, correction= 'none')
ppm214 <- ppm(pare20, ~ tot_vis+elev+water, hybrid20, correction= 'none')
ppm215 <- ppm(pare20, ~ pd_costpath+tot_vis+elev+water, hybrid20, correction= 'none')



#compare Gibbs models
ppm_AICc20 <- model.sel(ppm200, ppm201, ppm202, ppm203, ppm204, ppm205, ppm206, ppm207, ppm208, ppm209, ppm210, ppm211, ppm212, ppm213, ppm214, ppm215,rank=AICc)
ppm_AICc20
...
plot(intensity.ppm(ppm105), 
     col=gray.colors(10, start = 0.3, end = 0.9, gamma = 2.2, rev = T),
     main="") #riblab="Fitted intensity")
plot(pare20, pch=16, col='red', cex=0.5, add=T)
plot(pare10, pch=16, col='green', cex=0.5, add=T)
