...
pare15_points <- readOGR(here('Data', "Pare_Projected.shp"))
shore_line <- readOGR(here('Data', "shore limit.shp"))
DTM <- raster(here('Data', "tingrid_Copy.tif"))
path_distance <- raster(here('Data',"euc_.tif"))
ridge_distance <- raster(here('Data', ""))
...
pare <- ppp(pare15_points$POINT_X, pare15_points$POINT_Y, window=shore) #convert pare to a ppp object/ error? maybe use X_Cor
